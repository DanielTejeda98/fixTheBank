...
import splitPaymentMasterModel, {
  SplitPaymentMaster,
} from "@/models/splitPaymentMasterModel";
...
async function updateExpense(
  ...
  try {
    ...
    const updateRequest = {
      amount: request.amount,
      account: request.account
        ? new mongoose.Types.ObjectId(request.account)
        : null,
      category: request.category
        ? new mongoose.Types.ObjectId(request.category)
        : null,
...
async function reconcileExpense(
  reconciled: boolean,
  expenseId: mongoose.Types.ObjectId,
  userId: mongoose.Types.ObjectId
) {
...
async function createSplitExpense(
  ...
  try {
    ...
    let individualAmount =
      Math.round((request.amount / request.numberOfPayments) * 100) / 100;
    const expenses = [];
    ...
    for (let i = 0; i < request.numberOfPayments; i++) {
      ...
      if (
        i === request.numberOfPayments - 1 &&
        totalAmountCheck !== request.amount
      ) {
        const difference =
          Math.round((request.amount - totalAmountCheck) * 100) / 100;
      ...
      const expense = await expenseModel.create({
        ...
        description: request.description
          ? `${request.description} (Part ${i + 1} of ${
