...
import savingsAccount, {
  SavingsAccount,
  SavingsTransaction,
} from "@/models/savingsAccount";
import savingsAccountBucket, {
  SavingsBuckets,
} from "@/models/savingsAccountBucket";
import savingsModel, { PlannedSavings, Savings } from "@/models/savingsModel";
import { PlannedSaving } from "@/types/savings";
import mongoose from "mongoose";

export async function getAllSavingsDetails(
  ...
  try {
    ...
    const savingsDoc = await savingsModel
      .findById(userBudget.savings)
      .populate({
    ...
    if (!savingsDoc) {
      ...
      if (!newSavingsDoc) {
        throw Error(
          "Unable to create Savings document for budgetID: " + userBudget._id
        );
      }
    ...
    const totalSavings = savingsDoc._doc.savingsAccounts.reduce(
      (agg: number, curr: SavingsAccount) => (agg += curr.currentTotal),
      0
    );
...
export async function createSavingsAccount(
  ...
  try {
    ...
    const savingsDoc = (await savingsModel.findById(
      userBudget.savings
    )) as Savings;

    if (!savingsDoc) {
      throw Error(
        "No savings found with the following ID: " + userBudget.savings
      );
    }
...
export async function createSavingsTransaction(
  ...
  try {
    ...
    const currSavingsAccount = (await savingsAccount
      .findOne()
      .and([
...
export async function updateSavingsTransaction(
  ...
  try {
    ...
    console.log(currSavingsAccount.ledger);
    const transaction = currSavingsAccount.ledger.find(
      (trans) =>
        (trans._id as mongoose.Types.ObjectId).toString() === transactionId
    ) as SavingsTransaction;

    if (!transaction) {
      throw Error(
        "No transaction found under account " +
          currSavingsAccount._id +
          " for transaction ID: " +
          transactionId
      );
    }
    ...
    transaction.transactionType =
      request.transactionType || transaction.transactionType;
...
export async function createSavingsAccountBucket(
  userId: mongoose.Types.ObjectId,
  request: createSavingsAccountBucketRequest
) {
  try {
    await dbConnect();
    const userBudget = await findBudget(userId);
...
export async function getSavingsPlannerByMonth(
  userId: mongoose.Types.ObjectId,
  month: string
): Promise<PlannedSavings | undefined> {
  try {
    await dbConnect();
    const userBudget = await findBudget(userId);
...
export async function addSavingsPlan(
  userId: mongoose.Types.ObjectId,
  month: string,
  newSavingsPlanRequest: PlannedSaving
) {
  try {
    await dbConnect();
    const userBudget = await findBudget(userId);
    ...
    const plannedSavingsMonthList = savingsDoc.plannedSavings.find(
      (ps) => ps.month === month
    );
    ...
    if (!plannedSavingsMonthList) {
      savingsDoc.plannedSavings.push({
        month: month,
        savingsPlans: [mappedSavingsObj],
      });
    } else {
...
export async function removeSavingsPlan(
  userId: mongoose.Types.ObjectId,
  month: string,
...
export async function updateSavingsPlan(
  userId: mongoose.Types.ObjectId,
  month: string,
  updateSavingsPlanRequest: PlannedSaving
) {
  try {
    await dbConnect();
    const userBudget = await findBudget(userId);
    ...
    const sp = plannedSavingsMonthList.savingsPlans.find((sp) =>
      (sp._id as mongoose.Types.ObjectId).equals(updateSavingsPlanRequest._id)
    );

    if (!sp) {
      throw Error(
        `No savings plan found in ${month} with ${updateSavingsPlanRequest._id}`
      );
    }
