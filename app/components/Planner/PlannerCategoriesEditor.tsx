...
export default function PlannerCategoriesEditor({
  categories,
}: {
  categories: any[];
}) {
  ...
  const renderCategories = () => {
    ...
    return categories.map((category, index) => {
      return (
        <div
          className="flex justify-between items-center w-full rounded-md p-2 mb-2 last:mb-0"
          key={index}
        >
          <p>{category.name}</p>
          ...
          {addingNewCategory ? (
            ...
            >
              <input
                className="w-full"
                name="newCategory"
                onBlur={handleAddingCategoryBlur}
