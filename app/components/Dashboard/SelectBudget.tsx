...
const RENDER_YEARS_COUNT = new Date().getFullYear() - BASE_YEAR + FUTURE_YEARS + 1; // +1 to include for the zero index
...
export default function SelectBudget () {
    ...
    const [selectedMonth, setSelectedMonth] = useState(new Date(budgetDate).getMonth());
    const [selectedYear, setSelectedYear] = useState(new Date(budgetDate).getFullYear());
    ...
    const setDate = (date: Date) => {
        ...
        setBudgetDate(formatDateInput(new Date(date.getFullYear(), date.getMonth(), 2)));
    }
        ...
        <DrawerBody className="flex flex-col">
            <div className="flex items-center w-full gap-2">
                <div className="flex w-2/3 gap-2 flex-wrap">
                    <Label htmlFor="month" className="w-full">Budget Month</Label>
                    <Select name="month" value={selectedMonth.toString()} onValueChange={(value) => {
                        setSelectedMonth(parseInt(value));
                        // We set the day index to 2 to avoid timezone issues that may cause date to shift back a day
                        setBudgetDate(formatDateInput(new Date(selectedYear, parseInt(value), 2)));
                    }}>
                        <SelectTrigger className="w-full" >
                            <p>{new Date(0, selectedMonth).toLocaleString('en-US', { month: 'long' })}</p>
                        </SelectTrigger>
                        <SelectContent>
                            {[...Array(12)].map((_, index) => {
                                const monthValue = index.toString();
                                return (<SelectItem key={monthValue} value={monthValue}>
                                    <p>{new Date(0, index).toLocaleString('en-US', { month: 'long' })}</p>
                                </SelectItem>
                                )
                            })}
                        </SelectContent>
                ...
                <div className="flex w-1/3 gap-2 flex-wrap">
                    <Label htmlFor="year" className="w-full">Budget Year</Label>
                    <Select name="year" value={selectedYear.toString()} onValueChange={(value) => {
                        setSelectedYear(parseInt(value));
                        // We set the day index to 2 to avoid timezone issues that may cause date to shift back a day
                        setBudgetDate(formatDateInput(new Date(parseInt(value), selectedMonth, 2)));
                    }}>
                        <SelectTrigger className="w-full" >
                            <p>{selectedYear}</p>
                        </SelectTrigger>
            ...
            <Button type="button" onClick={handleSetToday}>Today</Button>
            <div className="flex w-full gap-2">
                <Button type="button" onClick={handleSetLastMonth} className="w-full" variant={'outline'}>Last Month</Button>
                <Button type="button" onClick={handleSetNextMonth} className="w-full" variant={'outline'}>Next Month</Button>
            </div>
        </DrawerBody>
