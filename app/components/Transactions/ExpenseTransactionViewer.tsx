...
export default function ExpenseTransactionViewer({ id }: { id: string }) {
  ...
  const transaction = useAppSelector(selectExpense).find(
  ...
  const account =
    useAppSelector(selectAccounts).find(
      (acc) => acc._id === transaction?.account
    )?.name || "";
  ...
  const handleOpenEditor = () => {
    setDrawerComponent(
      <ExpenseEditor budgetId={budgetId} transaction={transaction} />
    );
    setDrawerOpen(true);
  };
    ...
    <>
      ...
      <DrawerBody
        className={cn("flex flex-col gap-3", {
          "blur-sm": isHiddenGiftTransaction,
        })}
      >
