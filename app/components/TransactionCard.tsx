...
const TransactionCardContext = createContext<TransactionCardContext>(
  {} as TransactionCardContext
);

function TransactionCardProvider({
  children,
  isBlurredContent = false,
}: {
  children: ReactNode;
  isBlurredContent?: boolean;
}) {
...
function TransactionCardLeftBody({ children }: { children: ReactNode }) {
  const { isBlurredContent } = useContext(TransactionCardContext);
  return (
    <div
      className={cn(
        { "blur-sm": isBlurredContent },
...
function TransactionDescription({ children }: { children: ReactNode }) {
  return (
    <p className="text-sm text-start break-words max-w-[calc(100vw-3rem-12rem)] line-clamp-2">
      {children}
    </p>
  );
}

function TransactionCardRightBody({ children }: { children: ReactNode }) {
  const { isBlurredContent } = useContext(TransactionCardContext);
...
function TransactionCardDate({ date }: { date: Date }) {
  return (
    <p className="text-xs text-end text-neutral-400">
