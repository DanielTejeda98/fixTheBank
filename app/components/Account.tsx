...
export default function Account({closeDrawer}: {closeDrawer: Function}) {
    ...
    const getRequestersListResponse = async () => {
        try {
            ...
            reduxDispatch(setJoinRequestList({
                ...
                <NotificationCard key={requester._id} >
                    <div className="flex flex-wrap border-solid border-gray-500 border-b-2 pb-2">
                        ...
                        <div className="flex gap-3 mt-1">
                            <button className="text-sm bg-slate-500 rounded-md p-1" onClick={() => approveRequestee(requester._id)} disabled={pending}>Approve</button>
                            <button className="text-sm bg-red-700 rounded-md p-1" disabled={pending}>Deny</button>
                        </div>
                    </div>
                </NotificationCard>
            )
    ...
    return (
        <div>
            ...
            <div className="grid gap-3 mt-3 overflow-auto">
                {renderJoinRequests()}
            </div>
        </div>
    )
